...
class Stock {
    ...
    public function install() {
        ...
        $sqls=array();
        $sqls['stock']=array();
        $sqls['stock'][]="CREATE TABLE IF NOT EXISTS stock ( stock_id INT NOT NULL AUTO_INCREMENT, description VARCHAR(255) NULL, stock_code VARCHAR(255) NULL, notes BLOB NULL, quantity_held INT NULL, supplier_id INT NOT NULL, unit_cost DECIMAL(13,4) NULL, unit_gst DECIMAL(13,4) NULL, PRIMARY KEY (stock_id), INDEX fk_stock_supplier1_idx (supplier_id ASC), CONSTRAINT fk_stock_supplier1 FOREIGN KEY (supplier_id) REFERENCES supplier (supplier_id) ON DELETE NO ACTION ON UPDATE NO ACTION) ENGINE = InnoDB";
    ...
    public function get_page($page_string) {
        $result=NULL;
        switch ($page_string) {
            case "AddNewStockCodeItem":
                $result=$this->page_AddNewStockCodeItem();
                break;
            case "ReviewCurrentStock":
                $result=$this->page_ReviewCurrentStock();
                break;
            case "SubmitAddNewStockCode":
                $result=$this->submit_SubmitAddNewStockCode();
                break;
            case "ValidateAddNewStockCode":
                $result=$this->validate_ValidateAddNewStockCode();
                break;
        }
        return $result;
    }
    ...
    public function get_menus() {
        $result=array();
        $result['Stock']=array('New Stock Code|Add New Stock Code Item','Review Stock|Review Current Stock');
        return $result;
    }
    
    //handles the edit staff details page generation, includes the selector
    private function page_AddNewStockCodeItem() {
        $nsci=new BasicForm($this->db);
        $nsci->set_formname('Add New Stock Code');
        $nsci->set_heading('Add New Stock Code');
        $nsci->set_feature('Stock');
        $nsci->set_page_buttons(array('save','cancel'));
        $nsci->add_group('stock');
        $nsci->add_text_field_to_group('stock','stock code',300);
        $nsci->add_text_field_to_group('stock','description',300);
        $nsci->add_combo_field_to_group('stock','supplier','supplier|supplier_id|name');
        $nsci->add_textarea_field_to_group('stock','notes',4,50);
        $nsci->add_group('costs');
        $nsci->add_currency_field_to_group('costs','unit cost',100);
        $nsci->add_currency_field_to_group('costs','unit gst',100);
        
        $nsci->add_validation_to_field('stock','stock code');
        
        return $nsci->render_page();
    }
    
    private function submit_SubmitAddNewStockCode() {
        $description=   $this->db->clean($_REQUEST['description']);
        $notes=         $this->db->clean($_REQUEST['notes']);
        $stock_code=    $this->db->clean($_REQUEST['stock_code']);
        $supplier=      $this->db->clean($_REQUEST['supplier']);
        $unit_cost=     $this->db->clean($_REQUEST['unit_cost']);
        $unit_gst=      $this->db->clean($_REQUEST['unit_gst']);
                
        //get the supplier description
        $supplier_name=$this->db->query("select name from supplier where supplier_id=$supplier")[0]['name'];
        
        #insert the data
        $sql="INSERT INTO stock (description,stock_code,notes,quantity_held,supplier_id,unit_cost,unit_gst) VALUES ('$description','$stock_code','$notes',0,'$supplier','$unit_cost','$unit_gst')";
        $save_result=$this->db->query($sql);
        
        #either return the last event log or the error message
        if ($save_result=='') {
            $this->db->add_event("Stock code added: $stock_code, $description from $supplier_name");
    ...
    private function validate_ValidateAddNewStockCode() {
        $stock_code=    $this->db->clean($_REQUEST['stock_code']);
        $result=array();
        $stock_code_query=$this->db->query("select count(*) as number from stock where stock_code='$stock_code'")[0]['number'];
        if ($stock_code_query>0) {
            $result['stock_code']="This stock code already exists";
        }
        if ($stock_code=='') {
            $result['stock_code']="The stock code cannot be blank";
        }
